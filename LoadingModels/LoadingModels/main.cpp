...
//const std::string TEXTURE_PATH = "textures/concrete.png";
class HelloTriangleApplication {
	...
	void createGraphicsPipeline() {
		auto vertShaderCode = readFile("shaders/Shader06.vert.spv");
		auto fragShaderCode = readFile("shaders/Shader04.frag.spv");
	...
	void createDescriptorSetLayout() {
		...
		if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
			throw std::runtime_error("failed to craete descriptor set layout!");
		}*/
	...
	void createTextureImage() {
		...
		//stbi_uc* pixels = stbi_load("textures/galah.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	void loadModel() {
		...
		for (const auto& shape : shapes) {
			for (const auto& index : shape.mesh.indices) {
				...
				//vertIndices.push_back(static_cast<uint32_t>(vertIndices.size()));
	...
	void createIndexBuffer() {
		...
		void *data;
		vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertIndices.data(), (size_t)bufferSize);
		vkUnmapMemory(device, stagingBufferMemory);
